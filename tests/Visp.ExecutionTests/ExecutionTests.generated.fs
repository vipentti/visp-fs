...
module ``tests_arrays_array-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/arrays/array-0.visp"
...
module ``tests_arrays_array2d-1`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/arrays/array2d-1.visp"
...
module ``tests_attributes_let-attributes-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/attributes/let-attributes-0.visp"
...
module ``tests_for-in_for-in-1`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/for-in/for-in-1.visp"
...
module ``tests_functions_cons-concat-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/functions/cons-concat-0.visp"
...
module ``tests_include_include-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/include/include-0.visp"

module ``tests_include_include-1`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/include/include-1.visp"

module ``tests_include_include-macros-0-macros`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/include/include-macros-0-macros.visp"

module ``tests_include_include-macros-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/include/include-macros-0.visp"
...
module ``tests_macros_dbg-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/macros/dbg-0.visp"
...
module ``tests_macros_macro-lambda-shorthand-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/macros/macro-lambda-shorthand-0.visp"

module ``tests_macros_my-const-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/macros/my-const-0.visp"

module ``tests_macros_my-rinit-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/macros/my-rinit-0.visp"
...
module ``tests_macros_nested-macro-expansion-1`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/macros/nested-macro-expansion-1.visp"
...
module ``tests_macros_stringify-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/macros/stringify-0.visp"
...
module ``tests_macros_struct-macro-3`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/macros/struct-macro-3.visp"
...
module ``tests_macros_when-unless-example-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/macros/when-unless-example-0.visp"
...
module ``tests_parsing_keyword-strings-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/parsing/keyword-strings-0.visp"
...
module ``tests_parsing_operators-1`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/parsing/operators-1.visp"
...
module ``tests_parsing_span-to-int32-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/parsing/span-to-int32-0.visp"
...
module ``tests_parsing_strings-1`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/parsing/strings-1.visp"
...
module ``tests_parsing_strings-interpolation-1`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/parsing/strings-interpolation-1.visp"

module ``tests_parsing_strings-interpolation-2`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/parsing/strings-interpolation-2.visp"

module ``tests_parsing_strings-interpolation-3`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/parsing/strings-interpolation-3.visp"
...
module ``tests_pats_record-pat-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/pats/record-pat-0.visp"
...
module ``tests_type_lambda-shorthand-record-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/type/lambda-shorthand-record-0.visp"
...
module ``tests_type_object-exprs-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/type/object-exprs-0.visp"
...
module ``tests_type_record-2`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/type/record-2.visp"

module ``tests_type_record-3`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/type/record-3.visp"
...
module ``tests_type_record-mut-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/type/record-mut-0.visp"
...
module ``tests_type_union-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/type/union-0.visp"

module ``tests_type_val-ctor-0`` =
    [<Fact>]
    let ``can execute`` () = TestUtils.runTest "tests/type/val-ctor-0.visp"
