...
// Based on https://github.com/dotnet/fsharp/blob/main/src/Compiler/Utilities/range.fs
    ...
    member m.DebugCode =
        ...
        then
            ...
            try
                ...
                if not (File.Exists m.FileName) then
...
module Range =
    ...
    let rangeOrder =
        Order.orderOn
            (fun (r: range) -> r.FileName, (r.Start, r.End))
            (Pair.order (String.order, Pair.order (posOrder, posOrder)))
    ...
    let withStartEnd (startPos: Position) (endPos: Position) (r: range) =
        range (r.FileIndex, startPos, endPos)
    ...
    let mkFirstLineOfFile (file: string) =
        try
            ...
            let lines = File.ReadAllLines(file) |> Seq.indexed
